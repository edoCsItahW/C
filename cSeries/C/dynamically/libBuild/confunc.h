...
Array* quickSort(Array* arr) {
    ...
    for (int i = 0; i < arr->len; i++) {
        ...
        if (current < pivot) Arr.append(left, arr->data[i]);
        else if (current == pivot) Arr.append(middle, arr->data[i]);
        else Arr.append(right, arr->data[i]);
    }
...
int findPrime(int firstNum) {
    while (1) {
        for (int n = 1; n <= 4; n = (2 * n + 1))
            if (firstNum % n == n - 2) return firstNum;
