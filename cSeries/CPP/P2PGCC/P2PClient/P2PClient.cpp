...
SOCKET PrimaryUDP;  // 主UDP套接字
char UserName[10];  // 用户名
char ServerIP[20];  // 服务端IP地址
...
stUserListNode GetUser(char *username) {
    for (auto &UserIterator : ClientList)
...
void BindSock(SOCKET sock) {
    ...
    if (bind(sock, (struct sockaddr *)&sin, sizeof(sin)) < 0) throw Exception("bind error");
}
...
bool SendMessageTo(char *UserName, char *Message) {
    ...
    for (int i = 0; i < MAXRETRY; i++) {
        ...
        MessageHead.iMsgType   = P2PMESSAGE;
        ...
        transMessage.iMsgType = P2PTRANS;
