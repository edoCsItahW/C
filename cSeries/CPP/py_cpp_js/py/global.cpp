	...
	namespace Converter {
		...
		node::Array list(node::Env env, const py::object &value) {
			...
			for (int i = 0; i < list.size(); i++)
				arr[i] = Error::try_(pyToJs, env)(env, list[i]);
		...
		node::Object dict(node::Env env, const py::object &value) {
			...
			for (auto it : dict)
				obj.Set(it.first.cast<std::string>(), Error::try_(pyToJs, env)(env, it.second.cast<py::object>()));
		...
		node::Function function(node::Env env, const py::object &value) {
			return node::Function::New(env,
				[value, env](const node::CallbackInfo &info) -> node::Value {
					...
					return Error::try_(pyToJs, env)(env, result);
				},
		...
		node::Object module(node::Env env, const py::object &value) {
			...
			for (auto it : attrs) {
				...
				if (py::hasattr(value, k.c_str())) {
					...
					obj.Set(k, Error::try_(pyToJs, env)(env, v));
				}
			}
			return obj;
		}
		node::Function class_(node::Env env, const py::object &value, const std::string& type) {
			auto constructor = node::Function::New(env, [value, env](const node::CallbackInfo& info) -> node::Value {
				...
				for (auto it : attrs) {
					...
					if (py::hasattr(pyIns, k.c_str())) {
						...
						jsIns.Set(k, Error::try_(pyToJs, env)(env, v));
					}
				}
				return jsIns;
			...
			for (auto it : attrs) {
				...
				if (py::hasattr(value, k.c_str())) {
					auto v = py::getattr(value, k.c_str());
					constructor.Set(k, Error::try_(pyToJs, env)(env, v));
				}
			}
			return constructor;
		}
		...
		node::Value pyToJs(node::Env env, const py::object &value) {
			...
			if (py::isinstance<py::str>(value)) return Error::try_(str, env)(env, value);
			else if (py::isinstance<py::int_>(value)) return Error::try_(integer, env)(env, value);
			else if (py::isinstance<py::float_>(value)) return Error::try_(float_, env)(env, value);
			else if (py::isinstance<py::bool_>(value)) return Error::try_(boolean, env)(env, value);
			else if (py::isinstance<py::list>(value)) return Error::try_(list, env)(env, value);
			else if (py::isinstance<py::dict>(value)) return Error::try_(dict, env)(env, value);
...
namespace Api {
	node::Value import_(const node::CallbackInfo& info) {
		...
		return Error::try_(Py::Converter::pyToJs, info.Env())(info.Env(), module);
	}
