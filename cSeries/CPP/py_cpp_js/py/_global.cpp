...
namespace Error {
	void NotImplementedError(node::Env env, const std::string &msg) {
		node::Error::New(env, "NotImplementedError: " + msg).ThrowAsJavaScriptException();
	}

	void ArgumentError(node::Env env, const std::string& msg) {
		node::Error::New(env, msg).ThrowAsJavaScriptException();
	}

	Try::Try(node::Env env) : _env(env) {}
	Try::Try() = default;
	Try::~Try() = default;
	...
	namespace Tools {
		...
		py::tuple getArgs(const node::CallbackInfo &info) {
			...
			for (int i = 0; i < info.Length(); i++)
				args[i] = Js::Converter::jsToPy(info[i]);
	...
	namespace Converter {
		...
		node::Array list(node::Env env, const py::object &value) {
			...
			for (int i = 0; i < list.size(); i++)
				arr[i] = Error::try_(pyToJsR, env)(env, list[i]);
		...
		node::Object dict(node::Env env, const py::object &value) {
			...
			for (auto it : dict)
				obj.Set(it.first.cast<std::string>(), Error::try_(pyToJsR, env)(env, it.second.cast<py::object>()));

			return obj;
		}
		node::Function function(node::Env env, const py::object &value) {
			return node::Function::New(env,
		...
		node::Function class_(node::Env env, const py::object &value, const std::string& type) {
			auto constructor = node::Function::New(env, [value, env](const node::CallbackInfo& info) -> node::Value {
		...
		std::optional<node::Value> common(node::Env env, const py::object &value) {
			if (py::isinstance<py::str>(value)) return str(env, value);
			else if (py::isinstance<py::int_>(value)) return integer(env, value);
			else if (py::isinstance<py::float_>(value)) return float_(env, value);
			else if (py::isinstance<py::bool_>(value)) return boolean(env, value);
			...
			else if (value.is_none()) return none(env);
		...
		node::Value pyToJsR(node::Env env, const py::object &value) {
			...
			if (type == "str") return Error::try_(str, env)(env, value);
			else if (type == "int") return Error::try_(integer, env)(env, value);
			else if (type == "float") return Error::try_(float_, env)(env, value);
			else if (type == "bool") return Error::try_(boolean, env)(env, value);
			else if (type == "list") return Error::try_(list, env)(env, value);
			else if (type == "dict") return Error::try_(dict, env)(env, value);
			else if (std::ranges::contains(funcType, type)) return Error::try_(function, env)(env, value);
			else if (type == "module") return Error::try_(module, env)(env, value);
			else if (type == "NoneType" || value.is_none()) return none(env);
			else return Error::try_(class_, env)(env, value, type);
		}
		/* 循环版本的pyToJs */
				...
				if (py::isinstance<py::list>(cur)) {
					...
					for (auto it : dict) {
						...
						if (item.has_value())
							...
							if (Tools::isIterable(it.second.cast<py::object>()))
...
namespace Js::Converter {
	...
	py::list array(const node::Value &value) {
		...
		for (int i = 0; i < arr.Length(); i++)
			list.append(Error::try_(jsToPy, arr.Env())(arr[i]));
	...
	py::dict object(const node::Value &value) {
		...
		for (auto it = iter.begin(); it != iter.end(); ++it)
			dict[it.operator*().first.ToString().Utf8Value().c_str()] = Error::try_(jsToPy, obj.Env())(obj.Get(it.operator*().first));
	...
	py::object jsToPy(const node::Value &value) {
		if (value.IsString()) return Error::try_(string, value.Env())(value);
		else if (value.IsNumber()) return Error::try_(number, value.Env())(value);
		else if (value.IsBoolean()) return Error::try_(boolean, value.Env())(value);
		else if (value.IsArray()) return Error::try_(array, value.Env())(value);
		else if (value.IsObject()) return Error::try_(object, value.Env())(value);
		else if (value.IsFunction()) return Error::try_(function, value.Env())(value);
		else if (value.IsUndefined() || value.IsNull()) return null(value);
		else Error::NotImplementedError( value.Env(), "Unsupported type" + value.As<node::Object>().Get("constructor").As<node::Object>().Get("name").ToString().Utf8Value());
		return py::none();
	}
}
...
namespace Api {
	...
	void atexit(const node::CallbackInfo& info) {
		...
		if (std::atexit([]() { std::cout << "exit" << std::endl; }))
			return;
	}
}
